...
export class Creature {
  ...
  tick(dt: number): Pick<Command, "motors" | "eyes"> | undefined {
    ...
    if (!this.fixedGaze && t >= this.nextGaze) {
      this.gazeTarget = {
        x: (this.random() * 2 - 1) * 0.6,
        y: (this.random() * 2 - 1) * 0.4,
      };
      this.nextGaze = t + 600 + this.random() * 2200;
    }
    if (!this.fixedGaze) {
    ...
    const openness = this.sequence
      ...
      : blink < 240
        ...
        : 1;
    let yaw = this.restYaw + Math.sin(t / 4500) * 4 * this.idleGain,
      pitch =
        (this.status.behavior === "thinking" ? 6 : Math.sin(t / 3200) * 2) *
        this.idleGain;
    if (this.action) {
      ...
      const offset = gestureOffset(
        value.gesture,
        period ? ((t - start) % period) / period : 0,
      );
